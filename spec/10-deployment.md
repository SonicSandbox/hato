...
| 🚨 **D12 — in 1.0.2 AND 1.0.3** | Pressing *Run now* closes the window (Sonic, 2026-09-24, on two machines; four `0xc0000409` faults in his event log). Every gate above was green through it: no check pressed the button. **Fixed in the vault 2026-09-24, NOT released** — `RUNBOOK.md` §*LAYER 10*; the smoke now presses it (6b), red on the 1.0.3 zip. ⏸ `1.0.4` waits on Sonic's go |
