...
> | **The blacklist is built for length** | Dates, a count, a filter, capped scroll — and ⭐ **hato detects the entries whose video has rotated off disk and offers to remove them**, because *"it's likely they will not clean it"*. ▶ *As built (RUNBOOK 13j, 13z, 2026-09-25): the filter keeps a row whose file, folder or note holds EVERY word typed, in any case; the heading says "1 of 4 videos" while it filters; a line says when nothing matches; the notice about rows no longer on this machine steps aside while it filters, so its button never acts on rows it hides; the rows sit at the top of the capped box; and over an empty list there is no filter.* |
